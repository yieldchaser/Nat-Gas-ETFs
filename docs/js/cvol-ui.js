...
function renderScorecard(composites) {
    ...
    var rows = computeScorecard(composites, CvolState.regimeFilter);
    ...
    function binomPval(hits, n) {
        if (n < 3) return 1;
        var p0 = 0.5, z = (hits/n - p0) / Math.sqrt(p0*(1-p0)/n);
    ...
    html += '</tbody></table>';
    // ── Multi-Horizon Analysis sub-table ──
    ...
    if (horizonRows.length > 0) {
        html += '<div style="margin-top:18px;">' +
            ...
            '<th data-tooltip="21-day forward return hit rate and annualized Sharpe">21D HIT / SHARPE</th>' +
            '<th data-tooltip="42-day forward return hit rate and annualized Sharpe">42D HIT / SHARPE</th>' +
            '</tr></thead><tbody>';
        horizonRows.forEach(function(r) {
...
function renderLegacyRegimePanel(data, comp) {
    ...
    // 2. Vol Trend (5D ROC direction)
    var roc5 = null;
    if (n > 5 && last.ngvl != null && data[n-6].ngvl != null && data[n-6].ngvl > 0) {
    ...
    // VoV stability (high VoV = lower signal reliability, penalize)
    ...
    el.innerHTML =
        ...
        cell('CONVICTION', convLabel, convColor, tactical, convColor,
            'Weighted synthesis of 6 inputs: regime, skew, VRP, signals, term structure, VoV. Sub-label = prescriptive tactical read for current conditions.');
}
...
function renderSensitivityPanel(sensitivity) {
    ...
    var html = '<table class="scorecard-table" style="font-size:0.68rem;"><thead><tr>' +
        ...
        '<th data-tooltip="Number of events detected at this threshold across full history">N EVENTS</th>' +
        '<th data-tooltip="21D directional hit rate at this threshold">HIT 21D</th>' +
        '</tr></thead><tbody>';
    sensitivity.forEach(function(sig) {
...
function renderVarSeriesChips() {
    ...
    Object.keys(VAR_SERIES_CFG).forEach(function(k) {
        ...
        dot.style.cssText = 'display:inline-block; width:8px; height:8px; border-radius:1px; margin-right:6px; background:' + cfg.color + (['skewRatio','underlying','varSpread','wingDiv','regimeTension','skewRoc5'].indexOf(k) >= 0 ? '; height:2px; border-radius:0' : '');
...
(async function() {
    try {
        ...
        // Regime filter chips (scorecard only)
        document.getElementById('cvol-regime-filter').addEventListener('click', function(ev) {
            var chip = ev.target.closest('.signal-chip'); if (!chip) return;
            document.querySelectorAll('#cvol-regime-filter .signal-chip').forEach(function(c) { c.classList.remove('active'); });
            chip.classList.add('active');
            CvolState.regimeFilter = chip.dataset.regime;
            renderScorecard(CvolState.composites);
        });
